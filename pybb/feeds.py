...
class LastPosts(PybbFeed):
    ...
    def items(self):
        return Post.objects.filter(topic__forum__hidden=False, topic__forum__forum__hidden=False).order_by('-created')[:15]
...
class LastTopics(PybbFeed):
    ...
    def items(self):
        return Topic.objects.filter(forum__hidden=False, forum__forum__hidden=False).order_by('-created')[:15]
